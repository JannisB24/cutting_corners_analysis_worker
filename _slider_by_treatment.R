slider_by_treatment <- function(data_subset) {
  ...
  sliders_at_50_given_4060 <- numeric(nrow(data_subset))
  ...
  for(row in 1:nrow(data_subset)) {
    ...
    if(count_in_range > 0) {
      sliders_at_50_given_4060[row] <- count_at_50 / count_in_range
    } else {
      sliders_at_50_given_4060[row] <- 0
    }
  }
  
  data_subset$sliders_at_50_count <- sliders_at_50_per_obs
  data_subset$sliders_in_range_count <- sliders_in_range_per_obs
  data_subset$sliders_at_50_given_4060 <- sliders_at_50_given_4060
