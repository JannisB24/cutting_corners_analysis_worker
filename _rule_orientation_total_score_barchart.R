rule_orientation_total_score_barchart <- function(data, variable_name = "moral_foundations_20.1.player.moral_foundations_score", custom_title = "Moral Foundations Scores Distribution") {
  ...
  hist_result <- hist(scores, 
                      breaks = seq(-0.5, 72.5, by = 1),  # Bins centered on integers 0-60
                      plot = FALSE)
  ...
  barplot(counts, 
          main = custom_title,
          names.arg = 0:72,  # Show all scores from 0-60
          xlab = "Score (0-72)", 
          ylab = "Frequency",
          col = "steelblue",
  ...
  axis(1, at = seq(0, 72, by = 5) * 1.2, labels = seq(0, 72, by = 5))
